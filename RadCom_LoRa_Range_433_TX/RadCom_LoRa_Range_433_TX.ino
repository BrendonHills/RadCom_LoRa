...
#define RFM95_RST 4
#define RFM95_INT 7
#define LED A0 //if you wish to omit the external LED, resistor and sounder, merely replace 'A0' with '13' to use the onboard LED
...
void setup() {
  ...
  LoRa.setSpreadingFactor(7); //Set Spreading Factor 7 to 12. 7 is the fastest transmission time. 12 the slowest, working further below the noise floor
  LoRa.setCodingRate4(4); //Set coding rate for best signal immunity - range 4(5) - 4(8) - default 5 -  8 doubles TX time but more robust
...
void loop() {
  ...
  //flashing the led each time, incrementing the counter, waiting for two seconds and then sending the next packet, ad infinitum
  ...
  //flash led  and beep buzzer to show packet sent
